...
if (!empty($_GET['tag'])) {
    $see_moar = "http://www.epiktube.xyz/results.php?search=".$_GET['tag'];
} else if (!empty($_GET['user'])) {
  $see_moar = "http://www.epiktube.xyz/profile_videos?user=".$_GET['user'];  
}
if($singular != 'true') {
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
		<td> <?php  echo 'My Videos';  ?> // <span style="text-transform: capitalize;"><?php echo !empty($_GET['tag']) ? htmlspecialchars($_GET['tag']) : (!empty($_GET['user']) ? htmlspecialchars($_GET['user']) : (!empty($_GET['id']) ? htmlspecialchars($video['title']) : "")); ?>
</span><?php if ($singular != 'true' && $rlcount < 50) { echo ' // (' . $video_howmuch . ' ' . ($videos->rowCount() === 1 ? 'video' : 'videos') . ')'; } ?><?php if ($singular != 'true' && $rlcount > 50) { ?> // (over 50 videos, <a href="<?php echo htmlspecialchars($see_moar); ?>">see more</a>)<? } ?></td>
		<td align="right"><div style="vertical-align: text-bottom; text-align: right; padding: 2px;">
		<a href="http://www.epiktube.xyz" target="_blank"><img src="img/logo_rm2.png" width="38" height="15" border="0"></a>
	</tr>
</table>
</div>
<?php
    foreach($videos as $video) {
       $c = $conn->prepare("SELECT * FROM comments LEFT JOIN users ON users.uid = comments.uid WHERE vidon = ? AND users.termination = 0 AND is_reply = 0 ORDER BY post_date DESC");
$c->execute([$video['vid']]);
$fart = $c->rowCount();
        ?>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr valign="top">
		...
		<div class="moduleFrameDetails">Views: <?php echo $video['views']; ?> <br> Comments: <?php echo number_format($fart) ?></div>
		</td>    
	</tr>
</table>
</div>
