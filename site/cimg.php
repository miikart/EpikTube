...
// It basically takes into account all of the possibilities shown on cimg archives and crams them all into one PHP image generator, making it extremely accurate!
// This code is probably complex to work with; but ignore this. The important part is that if you want to use it, the variable to validate the captcha is $_SESSION['captcha'].
...
for ($y = 0; $y < $imageHeight; $y++) {
    $ratio = $y / $imageHeight;
    $r = (int)((1 - $ratio) * ($backgroundColorTop >> 16 & 0xFF) + $ratio * ($backgroundColorBottom >> 16 & 0xFF));
    $g = (int)((1 - $ratio) * ($backgroundColorTop >> 8 & 0xFF) + $ratio * ($backgroundColorBottom >> 8 & 0xFF));
    $b = (int)((1 - $ratio) * ($backgroundColorTop & 0xFF) + $ratio * ($backgroundColorBottom & 0xFF));
...
$textSize = 32;
$outlineSize = 1;
$textBox = imagettfbbox($textSize, 0, $_SERVER['DOCUMENT_ROOT'].'/img/ProFontWindows.ttf', $captchaText);
$textWidth = $textBox[2] - $textBox[0];
$textHeight = $textBox[3] - $textBox[5];
$textX = (int)(($imageWidth - $textWidth) / 2);
$textY = (int)(($imageHeight + $textHeight) / 2);
...
for ($x = $textX - $outlineSize; $x <= $textX + $outlineSize; $x++) {
    for ($y = $textY - $outlineSize; $y <= $textY + $outlineSize; $y++) {
        imagettftext($captchaImage, $textSize, 0, $x, $y, $outlineColor, $_SERVER['DOCUMENT_ROOT'].'/img/ProFontWindows.ttf', $captchaText);
    }
}

// Draw the actual text layer with aliased font
imageantialias($captchaImage, false);
imagettftext($captchaImage, $textSize, 0, $textX, $textY, $textColor, $_SERVER['DOCUMENT_ROOT'].'/img/ProFontWindows.ttf', $captchaText);
...
for ($y = 0; $y < $textHeight; $y++) {
    $ratio = $y / $textHeight;
    $r = (int)((1 - $ratio) * ($textColorTop >> 16 & 0xFF) + $ratio * ($textColorBottom >> 16 & 0xFF));
    $g = (int)((1 - $ratio) * ($textColorTop >> 8 & 0xFF) + $ratio * ($textColorBottom >> 8 & 0xFF));
    $b = (int)((1 - $ratio) * ($textColorTop & 0xFF) + $ratio * ($textColorBottom & 0xFF));
    $gradientTextColor = imagecolorallocatealpha($captchaImage, $r, $g, $b, rand(10, 100));
    imagettftext($captchaImage, $textSize, 0, $textX, $textY, $gradientTextColor, $_SERVER['DOCUMENT_ROOT'].'/img/ProFontWindows.ttf', $captchaText);
}
