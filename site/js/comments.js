...
function commentApproved(xmlHttpRequest)
{
	...
	if(response_code == "OK") {
		...
		return true;
            ...
            alert("An error occured while approved the comment. Please relogin and try again.");
	} else {
...
function handleStateChange(xmlHttpReq) {
	response_str = xmlHttpReq.responseText;
	setInnerHTML('all_comments_content', response_str);
...
function load_all_comments(video_id) {
	var remove_btn = document.getElementById('all_comments_button');
	remove_btn.value = "Loading Comments..";
	remove_btn.disabled = true
	
	getUrlAsync("/comment_servlet?get_comments&v=" + video_id, handleStateChange);
