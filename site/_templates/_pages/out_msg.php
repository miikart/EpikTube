...
<table width="100%" align="center" cellpadding="1" cellspacing="1" border="0" bgcolor="#EEEEEE">
	<tbody>
	<tr>
	<td width="100%"><img src="img/pixel.gif" width="1" height="5"> <? if($read_schedule['before'] != NULL) { ?><a href="out_msg.php?id=<?= $read_schedule['before'] ?>"><< Prev</a><? } ?><? if($read_schedule['before'] != NULL && $read_schedule['next'] != NULL) { ?>&nbsp;|&nbsp;<? } ?><? if($read_schedule['next'] != NULL) { ?><a href="out_msg.php?id=<?= $read_schedule['next'] ?>">Next >></a><? } ?></td>
	</tr>
	<tr>
