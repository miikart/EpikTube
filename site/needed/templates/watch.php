...
<div id="flashcontent">
        ...
        <?php if (isset($_SESSION['uid']) && $session['staff'] == 1 && $session['uid'] != $video['uid']) { ?><p>Moderation: <a style="color:#f22b33;" href="/admin/feature?video_id=<?php echo htmlspecialchars($video['vid']); ?>">Feature This!</a>&nbsp;&nbsp;//&nbsp;&nbsp;<a style="color:#f22b33;" href="/admin/remove_video?user=<?php echo htmlspecialchars($uploader['username']); ?>">Remove Video</a>&nbsp;&nbsp;//&nbsp;&nbsp;<a style="color:#f22b33;" href="/age_restrict?v=<?php echo htmlspecialchars($video['vid']); ?><?php if($video['agerestrict'] == 0) { echo"&restrict"; } ?>"><?php if($video['agerestrict'] == 0) { ?>Age Restrict Video<?php } else { echo"Remove Age Restriction"; } ?></a>
        <? } ?>
				</div>
...
<div>
	...
	<table width="485" cellpadding="0" cellspacing="0" border="0" align="center">
		<tr valign="top">
			...
			<div style="padding-bottom: 10px;"><span style="background-color: #FFFFAA; padding: 2px;">Tags:</span>&nbsp; <?php $tags = explode(" ", $video['tags']); $tagCount = count($tags); foreach ($tags as $index => $tag) { echo '<a      href="results?search=' . htmlspecialchars($tag) . '">' . htmlspecialchars($tag) . '</a>';
    if ($index < $tagCount - 1) {
        echo ', ';
    }
}
