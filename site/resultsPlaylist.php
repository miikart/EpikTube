...
$start_time = microtime(true);
  ...
  $vidocount = $conn->prepare("SELECT * FROM playlists LEFT JOIN users ON users.uid = playlists.uid WHERE (playlists.description REGEXP ? OR playlists.title REGEXP ? OR users.username REGEXP ?) AND users.termination = 0 AND playlists.action = 'create'  ORDER BY created_at DESC");
    $vidocount->execute([$search, $search, $search]);
...
$offs = ($page - 1) * $ppv;
  $videos = $conn->prepare("SELECT * FROM playlists LEFT JOIN users ON users.uid = playlists.uid WHERE (playlists.description REGEXP ? OR playlists.title REGEXP ? OR users.username REGEXP ?) AND users.termination = 0   AND playlists.action = 'create'  ORDER BY created_at DESC LIMIT $ppv OFFSET $offs");
    $videos->execute([$search, $search, $search]);   
